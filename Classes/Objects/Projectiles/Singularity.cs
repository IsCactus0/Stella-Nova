...
namespace Stella_Nova.Classes.Objects.Projectiles
{
    public class Singularity : Projectile
    {
        ...
        public Singularity(Projectile projectile)
            : base(projectile)
        {
            BurnTime = 0.5f;
        }
                ...
                for (int i = 0; i < asteroids.Count; i++)
                {
                    ...
                    if (isCrit)
                    {
                        ...
                        ParticleManager.particles.Add(new ScoreIndicator(
                            ...
                            0f,
                            0f, 0.01f, 1.8f, finalDamage.ToString(), 0.15f + (finalDamage / (MaxDamage * CritBonus) / 10f), Color.Lerp(Color.Crimson, Color.MediumVioletRed, finalDamage / (MaxDamage * CritBonus)), Main.Roboto_Medium));
                    }
                    else
                    {
                        ParticleManager.particles.Add(new ScoreIndicator(
                            ...
                            ((float)Main.random.NextDouble() - 0.5f) * 0.75f,
                            0f,
                            0f, 0.01f, 1.5f, finalDamage.ToString(), 0.1f + (damage / MaxDamage / 10f), Color.Lerp(Color.MistyRose, Color.DarkOrange, damage / MaxDamage), Main.Roboto_Light));
                    }
