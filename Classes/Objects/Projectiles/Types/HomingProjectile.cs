...
namespace Stella_Nova.Classes.Objects.Projectiles
{
    ...
    {
        public HomingProjectile(Projectile projectile) : base(projectile)
        {
        ...
        public HomingProjectile(float x, float y, float vx, float vy, float ax, float ay, float angle, float vangle, float aangle, float mass, float friction, int playerIndex, int penetration, int minDamage, int maxDamage, float critChance, float critBonus, float maxLifespan, float trackingStrength, SpaceObject target)
            : base(x, y, vx, vy, ax, ay, angle, vangle, aangle, mass, friction, playerIndex, penetration, minDamage, maxDamage, critChance, critBonus, maxLifespan)
        {
            TrackingStrength = trackingStrength;
            Target = target;
        ...
        {
            if (Target is null && TeamIndex != 1 && Vector2.Distance(PlayerManager.player.Position, this.Position) < 1024)
                Target = PlayerManager.player;
            if (Target is null)
                Target = EntityManager.FindNearest(Position, TeamIndex, 1024);
            if (Target is null)
                Target = AsteroidManager.FindNearest(Position, 1024);
        ...
        public override void Draw(SpriteBatch spriteBatch)
        {
            if (!(Target is null))
            {
                ...
                {
                    Vector2 toTarget = Target.Position - this.Position;
                    toTarget.Normalize();
                    Drawing.DrawLine(spriteBatch, Target.Position - toTarget * ((Asteroid)Target).Radius, this.Position, Color.Crimson, 3);
                }
                else Drawing.DrawLine(spriteBatch, Target.Position, Position, Color.Coral, 3);
                Target.Polygon.Draw(spriteBatch, Color.Coral, 5f);
            }
