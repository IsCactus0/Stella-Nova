...
namespace Stella_Nova.Classes.Objects.Projectiles
{
    public class Shockwave : Projectile
    {
        ...
        public Shockwave(Projectile projectile)
            : base(projectile)
        {
            Radius = 0f;
            MaxRadius = 256f;
        ...
        public override void FindCollisions()
        {
            List<SpaceObject> spaceObjects = AsteroidManager.FindAll(Position, Radius)
                        ...
                        .Append(PlayerManager.player).ToList();

            for (int i = 0; i < spaceObjects.Count; i++)
            {
                Vector2 force = spaceObjects[i].Position - Position;
                float distance = force.Length();
                if (distance <= Radius)
                    spaceObjects[i].Acceleration += (spaceObjects[i].Position - Position) * Strength / (Mass + 1) / distance;
            }
        }
