...
namespace Stella_Nova.Classes.Objects
{
    public class Polygon
    {
        ...
        public void Draw(SpriteBatch spriteBatch, Color colour, float strokeWeight = 3f)
        {
            ...
            {
                Drawing.DrawLine(spriteBatch, Utility.VectorXY(CurrentVertices[CurrentVertices.Count - 1] + Origin), Utility.VectorXY(CurrentVertices[0] + Origin), colour, strokeWeight);
                for (int v = 0; v < CurrentVertices.Count - 1; v++)
                {
                    // Drawing.DrawLine(spriteBatch, Utility.VectorXY(CurrentVertices[v] + Origin), Utility.VectorXY(Origin), Color.Red, 3f);
                    Drawing.DrawLine(spriteBatch, Utility.VectorXY(CurrentVertices[v] + Origin), Utility.VectorXY(CurrentVertices[v + 1] + Origin), colour, strokeWeight);
                }
            }
        }
        public void DrawWithLighting(SpriteBatch spriteBatch, Color colour, float strokeWeight = 3f)
        {
            if (CurrentVertices.Count > 0)
            {
                float averageZ = (CurrentVertices[CurrentVertices.Count - 1].Z + CurrentVertices[0].Z) / 2f;
                Vector2 averageVert = Utility.VectorXY(CurrentVertices[CurrentVertices.Count - 1] + CurrentVertices[0]) / 2f;
                Color lineColour = colour * (Utility.Map(averageZ, MaxZDepth, -MaxZDepth, 0.25f, 1.5f) + CalculateLight(averageVert, Main.sunAngle));
                // Drawing.DrawLine(spriteBatch, Utility.VectorXY(CurrentVertices[CurrentVertices.Count - 1] + Origin), Utility.VectorXY(Origin), Color.Red, 1f);

                Drawing.DrawLine(spriteBatch, Utility.VectorXY(CurrentVertices[CurrentVertices.Count - 1] + Origin), Utility.VectorXY(CurrentVertices[0] + Origin), lineColour, strokeWeight);
                for (int v = 0; v < CurrentVertices.Count - 1; v++)
                {
                    // Drawing.DrawLine(spriteBatch, Utility.VectorXY(CurrentVertices[v] + Origin), Utility.VectorXY(Origin), Color.Red, 1f);

                    averageZ = (CurrentVertices[v].Z + CurrentVertices[v + 1].Z) / 2f;
                    averageVert = Utility.VectorXY(CurrentVertices[v] + CurrentVertices[v + 1]) / 2f;
                    lineColour = colour * (Utility.Map(averageZ, MaxZDepth, -MaxZDepth, 0.25f, 1.5f) + CalculateLight(averageVert, Main.sunAngle));
                    Drawing.DrawLine(spriteBatch, Utility.VectorXY(CurrentVertices[v] + Origin), Utility.VectorXY(CurrentVertices[v + 1] + Origin), lineColour, strokeWeight);
                }
            }
        }
        ...
        public void ApplyTransforms()
        {
            ...
            {
                CurrentVertices[i] = Vector3.Transform(OriginalVertices[i], scale);
        ...
        public void Translate(float moveX, float moveY)
        {
            for (int i = 0; i < CurrentVertices.Count; i++)
                CurrentVertices[i] += new Vector3(moveX, moveY, 0);
        }
        ...
        {
            float vertAngle = (float)Math.Atan2(vertice.Y, vertice.X);
            float difference = (Math.Abs(vertAngle) - sunAngle) / MathHelper.Pi - 0.25f;
        ...
        public void ResetVertices(float angle)
        {
