            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                ParticleManager.particles.Add(
                    new Spark(
                        ...
                        Color.SeaGreen, Color.Aquamarine, Color.DarkSlateGray));
            }
        }
        public override void CreateNewProjectile(Vector2 pos, Vector2 vel, float angle, int teamIndex)
        {
            Projectile temp = new Laser(pos.X, pos.Y, vel.X, vel.Y, 0f, 0f, angle, 0f, 0f, 1f, 0.8f, teamIndex, 1, 5, 14, 0.23f, 1.4f, 0.15f + ((float)Main.random.NextDouble() * Spread));
