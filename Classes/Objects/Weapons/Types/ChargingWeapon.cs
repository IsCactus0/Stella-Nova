...
namespace Stella_Nova.Classes.Objects.Weapons
{
    public abstract class ChargingWeapon : EnergyWeapon
    {
        ...
        public override void Update(float delta, bool trigger, bool reload, SpaceObject source, int teamIndex)
        {
            if (trigger)
            {
                ...
                if (Utility.CalculateChancePerSecond(delta, 3f))
                {
                    ...
                    {
                        Vector2 tempPos = source.Position + Utility.RotateVector(barrel, source.Angle);
        ...
        {
            // Draw Ammunition
            Vector2 loadedStringSize = Main.Roboto_Medium.MeasureString($"{Loaded}") * 0.4f;
            Vector2 totalStringSize = Main.Roboto_Light.MeasureString($"/{Ammunition}") * 0.2f;
            Vector2 chargeStringSize = Main.Roboto_Light.MeasureString("/" + (Charge / ChargeTime).ToString("0.00")) * 0.2f;

            // Loaded
            spriteBatch.DrawString(
                Main.Roboto_Medium, $"{Loaded}",
                new Vector2(
                    Main.graphics.PreferredBackBufferWidth - loadedStringSize.X - 200,
                    Main.graphics.PreferredBackBufferHeight - loadedStringSize.Y- 64),
                Color.PowderBlue, 0f, Vector2.Zero, 0.4f, SpriteEffects.None, 0);
            // Ammunition
            spriteBatch.DrawString(
                Main.Roboto_Light, $"/{Ammunition}",
                new Vector2(
            ...
            // Charge
            spriteBatch.DrawString(
                Main.Roboto_Light, " " + (Charge / ChargeTime).ToString("0.00"),
                new Vector2(
