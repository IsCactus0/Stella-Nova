    ...
    public class HomingMissile : Weapon
    {
        ...
        public override void CreateMuzzleFlash(Vector2 pos, Vector2 vel, float angle, int teamIndex)
        {
            ...
            new Blur(
                ...
                (float)Main.random.NextDouble() - 0.5f,
                0f, 0.1f, 0.7f, (float)Main.random.NextDouble() + 0.2f, teamIndex, Main.random.Next(24, 32), Color.DimGray, Color.Black));
            ParticleManager.particles.Add(
                new Spark(
                    pos.X, pos.Y,
                    vel.X, vel.Y,
                    ...
                    (float)Main.random.NextDouble() - 0.5f,
                    (float)Main.random.NextDouble() - 0.5f,
                    0f, 0.1f, 1f, 0.1f, teamIndex, Main.random.Next(32, 40), Color.DarkOrange, Color.LightGoldenrodYellow, Color.Maroon));
        ...
        {
            Projectile temp = new Plasma(pos.X, pos.Y, vel.X, vel.Y, 0f, 0f, angle, 0f, 0f, 1f, 0.8f, teamIndex, 1, 5, 14, 0.23f, 1.9f, 3f, 200f);
