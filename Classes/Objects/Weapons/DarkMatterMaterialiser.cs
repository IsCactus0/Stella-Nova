...
namespace Stella_Nova.Classes.Objects.Weapons
{
    public class DarkMatterMaterialiser : EnergyWeapon
    {
        ...
        public override void CreateMuzzleFlash(Vector2 pos, Vector2 vel, float angle, int teamIndex)
        {
            ...
            ParticleManager.particles.Add(
                new Spark(pos.X, pos.Y,
                    ...
                    (float)Main.random.NextDouble() - 0.5f,
                    (float)Main.random.NextDouble() - 0.5f,
                    0f, 0.1f, 1f, 0.1f, teamIndex, Main.random.Next(32, 40),
        ...
        {
            Projectile temp = new DarkMatter(pos.X, pos.Y, vel.X, vel.Y, 0f, 0f, angle, 0f, 0f, 1f, 1f, teamIndex, 1,
                46, 129, 0.7f, 1.8f,
                3f);
            ProjectileManager.projectiles.Add(temp);

            float randomAngle = ((float)Main.random.NextDouble() - 0.5f) * MathHelper.TwoPi;
            Barrels = new List<Vector2>() { new Vector2((float)Math.Cos(randomAngle), (float)Math.Sin(randomAngle)) * 100f };
        }
