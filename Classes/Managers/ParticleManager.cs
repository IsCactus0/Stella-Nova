...
namespace Stella_Nova.Classes.Managers
{
    public class ParticleManager : DrawableGameComponent
    {
        ...
        {
            spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.Additive, null, null, null, null, transformMatrix: Main.camera.Transform);
            for (int i = 0; i < particles.Count; i++)
                if (particles[i] is ScoreIndicator || Utility.WithinBounds(particles[i], Main.camera, Main.RenderDistance))
