...
namespace Stella_Nova.Classes.Managers
{
    public class AsteroidManager : DrawableGameComponent
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (asteroids.Count <= 228 / Main.camera.Zoom)
                SpawnAsteroid(4, 5);
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            for (int i = 0; i < asteroids.Count; i++)
                if (Utility.WithinBounds(asteroids[i], Main.camera, Main.RenderDistance))
        ...
        public static List<Asteroid> SplitSimple(Asteroid asteroid)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                Vector2 randomOffset = new Vector2(
                    (float)Main.random.NextDouble() * asteroid.Radius * (float)Math.Cos(Main.random.NextDouble() * Math.PI * 2),
                    (float)Main.random.NextDouble() * asteroid.Radius * (float)Math.Sin(Main.random.NextDouble() * Math.PI * 2));
