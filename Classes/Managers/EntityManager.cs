...
namespace Stella_Nova.Classes.Managers
{
    public class EntityManager : DrawableGameComponent
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            // if (entities.Count < 5)
            // {
            //     Vector2 spawn = Utility.RandomAlongBorder(Main.camera.BoundingBox, 32, Main.RenderDistance);
            //     entities.Add(new Drone(spawn.X, spawn.Y));
            // }
            
            float delta = ((float)gameTime.ElapsedGameTime.Ticks / TimeSpan.TicksPerSecond) * Main.GameSpeed;
            // if (entities.Contains())//Utility.CalculateChancePerSecond(delta, 0.0001f))
            // {
            //     Vector2 spawn = Utility.RandomAlongBorder(Main.camera.BoundingBox, 32, Main.RenderDistance);
            //     entities.Add(new Blackhole(spawn.X, spawn.Y));
            // }
